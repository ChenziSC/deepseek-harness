...
The launcher and shipped Web and Headless providers write their human-facing descriptions, examples, and usage diagnostics in Simplified Chinese. Command names, flags, profile ids, exit codes, config-dump output, and arguments or output forwarded to another program remain unchanged; each out-of-tree app still owns the language of its own command copy.
