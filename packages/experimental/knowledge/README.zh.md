...
`request.query` 必须非空，`request.maxResults` 必须是正整数。`request.strategy` 可以请求 BM25、Dense 或 Hybrid 召回、自动或具体的 Dense 索引，以及自动、开启或关闭重排序。提供方应用部署策略，并随排序命中返回实际执行的高层策略。每条命中包含来源文档与分片标识、可选标题与来源标签、原始分片正文，以及仅能在本次响应内比较的有限分数。
