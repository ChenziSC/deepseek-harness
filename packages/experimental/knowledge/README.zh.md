...
`request.query` 必须非空，`request.maxResults` 必须是正整数。每条命中包含来源文档与分片标识、可选标题与来源标签、原始分片正文，以及仅能在本次响应内比较的有限分数。

`KnowledgeError` 提供 `KNOWLEDGE_INVALID_REQUEST`、`KNOWLEDGE_CANCELLED` 和 `KNOWLEDGE_SEARCH_FAILED`。提供方内部的文件与模型细节不会进入公共错误类型。
