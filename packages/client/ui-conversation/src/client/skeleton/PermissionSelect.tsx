...
/* 盾牌图标来自 design set 1556：勾号表示 read-only，铅笔表示 workspace-write，
   感叹号表示 full access。使用 currentColor 后，触发器和菜单行可分别控制颜色。 */
...
/** 根据权限机器值选取图标；Host 自定义值若不在设计集合中则不显示图标。 */
function permissionGlyph(value: string): ReactNode | undefined {
  return permissionGlyphs[value]
}

/**
 * 展示转换：kebab-case 机器名转为标题格式；内置 preset 改用 locale 产品标签，
 * Host 配置的非 kebab 名称原样透传。
 */
...
function optionLabel(option: PermissionSelectValue['options'][number], t: ComposerBarProps['t']): string {
  const builtInName = BUILT_IN_PERMISSION_NAMES.get(option.value)
  if (builtInName !== undefined && (option.name === option.value || option.name === builtInName)) {
      ...
      <Menu
        ...
        anchor={
          ...
          >
            ...
            {/* 与同级 ModelSelect 触发器使用相同图标和展开旋转效果。 */}
