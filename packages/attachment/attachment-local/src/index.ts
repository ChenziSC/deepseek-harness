/** 以 `DSH_HOME` 下方为根目录的本地持久附件后端。 @module @deepseek-ai/dsh-attachment-local */
  ...
  wait(signal?: AbortSignal): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      void this.promise.then((value) => {
        ...
        signal.removeEventListener('abort', abort)
        release(false)
        // CompressionLimiter 会在到达此处理器前规范化任务拒绝原因。
...
/**
 * 持久内容寻址的本地附件 store。准入、持久规范化和路由请求派生共享同一
 * 实例级并发上限；同一 `variantId` 的请求转换还会合并为一次工作。
 */
